...
好了，现在我们来总结一下RLHF的步骤吧，它大致如下：

•	第一步（Actor 登场）： Actor 网络（也就是我们要训练的 LLM本体）根据 Prompt，一个 Token 一个 Token 地生成了一句完整的回答：“从前有座山...”。
•	第二步（RM 出场 —— 核心环节！）： 这时，Actor 停止生成。系统把 Prompt + 生成的完整回答组合在一起，整体输入给冻结的 RM（奖励模型）。RM 根据之前用 BT 模型训练出的偏好标准，给这个完整的句子打出一个标量分数 ‭$r$‬（比如 8.5 分）。
•	第三步（加入 Ref 限制）： 像我们上一个问题聊过的，系统会用冻结的 Ref 模型计算 KL 散度惩罚，把 RM 给的原始分数 ‭$r$‬ 变成最终奖励 ‭$R$‬。
•	第四步（Critic 登场 & 算账）： * Critic 的任务是预测每个 Token 的价值。现在完整的回答生成完了，最终的真实奖励 ‭$R$‬ 也由 RM 发放下来了。
•	Critic 终于可以核对答案了：它会将自己之前对每个状态预测的 ‭$V$‬ 值，与 RM 发放的真实 ‭$R$‬（通过广义优势估计 GAE 向回传导）进行对比，计算 MSE 损失来更新自己。
•	同时，利用真实的 ‭$R$‬ 和预测的 ‭$V$‬，计算出优势函数 ‭$A(s,a)$‬‭‬‭‬‭‬‭‬‭‬。
•	第五步（Actor 更新）： Actor 利用算出的 ‭$A(s,a)$‬‭‬‭‬‭‬‭‬‭‬，更新自己的策略参数。
